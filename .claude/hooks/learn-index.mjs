...
function validate(note, schema) {
  ...
  for (const k of required) {
    if (
      fm[k] === undefined ||
      fm[k] === "" ||
      (Array.isArray(fm[k]) && fm[k].length === 0 && k === "tags")
    ) {
  ...
  if (fm.category) {
    ...
    if (dirname(note.path) !== expectedDir) {
      errors.push(
        `category="${fm.category}" não bate com pasta-pai (esperado ${relative(PROJECT_ROOT, expectedDir)})`,
      );
    }
  }
  ...
  if (fm.recurrence && !schema.recurrence.has(fm.recurrence)) {
    errors.push(
      `recurrence="${fm.recurrence}" inválido (use baixa|media|alta)`,
    );
  }
...
if (!schema) {
  process.stderr.write(
    "learn-index.mjs: _SCHEMA.md não encontrado em .claude/learn/. Pulando regeneração.\n",
  );
  process.exit(1);
}
