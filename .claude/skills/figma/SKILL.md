...
> **Para página inteira nova (5+ seções)**, prefira o workflow orquestrado em **4 fases CMS-first**:
>
> 1. `/build-prep <figma-url>` — gera manifesto + yaml schema Directus + assets (Partes A+B desta skill, automatizadas)
> 2. `/build-cms <page>` — cria coleções no Directus a partir do yaml + sobe seeds (texto literal e assets do Figma) + gera helpers em `src/lib/cms/{page}.js`
> 3. `/build-components <page>` — implementa `src/components/{Comp}/` shared a partir das specs do manifesto (serial topológico)
> 4. `/build-page <page>` — lança subagentes `section-builder` em paralelo (Parte C desta skill, isolada por subagente), consumindo componentes prontos + dados Directus
>
> Esta skill continua sendo a referência técnica subjacente — o `/build-prep` segue as Partes A+B,
> e cada `section-builder` segue a Parte C. Use esta skill diretamente apenas para:
>
...
4. Registrar classes `text-{categoria}-{numero}` como plugin Tailwind (se faltarem)
