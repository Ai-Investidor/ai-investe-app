...
function findIconInstances(node, opts, results = []) {
  ...
  if (
    ...
    const skipNames =
      /^(Badge|Button|Tab|Input|Avatar|pagination|Text|Divider|Breadcrumb)/i;
...
function cleanSvg(svg, keepColors) {
  ...
  if (!keepColors) {
    ...
    svg = svg.replace(
      /fill="#(?!fff|FFF|ffffff|FFFFFF)[0-9a-fA-F]{3,8}"/g,
      'fill="currentColor"',
    );
  }
...
async function main() {
  ...
  const nameFilter = args["name-filter"]
    ? new RegExp(args["name-filter"], "i")
    : null;
  ...
  for (const [nodeId, nodeData] of Object.entries(nodesData.nodes || {})) {
    ...
    const icons = findIconInstances(nodeData.document, {
      minSize,
      maxSize,
      nameFilter,
    });
  ...
  for (const icon of icons) {
    console.log(
      `   • ${icon.safeName} ← "${icon.name}" (${icon.width}x${icon.height})`,
    );
  }
  ...
  for (let i = 0; i < icons.length; i += BATCH_SIZE) {
    ...
    console.log(
      `   Batch ${Math.floor(i / BATCH_SIZE) + 1}: ${batch.length} icons...`,
    );
    ...
    for (const icon of batch) {
      ...
      try {
        ...
        console.warn(
          `   ⚠️ Download failed for "${icon.safeName}": ${err.message}`,
        );
      }
    }
  }
    ...
    if (svgMap[icon.id]) {
      if (format === "svg-files" || format === "both") {
        ...
        const componentName = icon.safeName
          ...
          .join("");
        console.log(
          `import ${componentName} from 'assets/icons/${icon.safeName}.svg?react'`,
        );
      }
      if (format === "symbols") {
        console.log(
          `// (sprite output — não é o padrão deste projeto, prefira --format svg-files + ?react)`,
        );
